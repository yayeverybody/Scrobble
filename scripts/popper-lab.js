...
function resize(){const w=host.clientWidth||innerWidth,h=host.clientHeight||innerHeight;r.setSize(w,h,false);camera.aspect=w/h;camera.position.set(7.6,8.5,10.5);camera.lookAt(0,-.05,0);camera.zoom=1;camera.updateProjectionMatrix();const fitWidth=9.9,visibleWidth=2*Math.tan(T.MathUtils.degToRad(camera.fov*.5))*camera.position.distanceTo(new T.Vector3(0,-.05,0))*camera.aspect;camera.zoom=Math.min(1,visibleWidth/fitWidth);camera.updateProjectionMatrix()}addEventListener('resize',resize);resize();
