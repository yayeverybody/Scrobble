...
const camera=new T.PerspectiveCamera(34,1,.1,100);camera.position.set(7.6,8.5,10.5);camera.lookAt(0,-.05,0);camera.zoom=1;
...
function resize(){const w=Math.max(1,host.clientWidth),h=Math.max(1,host.clientHeight);r.setSize(w,h,false);camera.aspect=w/h;const target=new T.Vector3(0,-.35,0),dir=new T.Vector3(7.6,8.5,10.5).sub(new T.Vector3(0,-.05,0)).normalize();camera.fov=34;camera.zoom=1;camera.updateProjectionMatrix();const vf=T.MathUtils.degToRad(camera.fov),hf=2*Math.atan(Math.tan(vf/2)*camera.aspect),fitRadius=5.15,dist=fitRadius/Math.sin(Math.min(vf,hf)/2)*1.08;camera.position.copy(target).addScaledVector(dir,dist);camera.lookAt(target);camera.updateProjectionMatrix()}addEventListener('resize',resize);resize();
