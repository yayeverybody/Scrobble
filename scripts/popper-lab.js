...
function resize(){const w=Math.max(1,host.clientWidth),h=Math.max(1,host.clientHeight);r.setSize(w,h,false);camera.aspect=w/h;camera.position.set(7.6,8.5,10.5);camera.lookAt(0,-.05,0);camera.zoom=.72;camera.updateProjectionMatrix()}addEventListener('resize',resize);resize();
