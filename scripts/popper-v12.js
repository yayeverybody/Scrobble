...
// Classic race-board geometry: 28 shared clockwise track spaces, four colored START spaces, and four 4-space FINISH lanes.
const trackEl=document.getElementById('track');
const side=[];for(let i=0;i<8;i++)side.push([8+i*(84/7),8]);for(let i=1;i<7;i++)side.push([92,8+i*14]);for(let i=7;i>=0;i--)side.push([8+i*(84/7),92]);for(let i=6;i>=1;i--)side.push([8,8+i*14]);
side.forEach(([x,y],i)=>{const s=document.createElement('i');s.className='space'+([0,7,14,21].includes(i)?` start c${[0,7,14,21].indexOf(i)}`:'');s.style.left=x+'%';s.style.top=y+'%';trackEl.appendChild(s)});
const finishCoords=[[[20,20],[27,27],[34,34],[41,41]],[[80,20],[73,27],[66,34],[59,41]],[[80,80],[73,73],[66,66],[59,59]],[[20,80],[27,73],[34,66],[41,59]]];
...
const r=new T.WebGLRenderer({antialias:true,alpha:false}); r.setPixelRatio(Math.min(2,devicePixelRatio||1)); r.shadowMap.enabled=false; host.appendChild(r.domElement); const miniR=new T.WebGLRenderer({antialias:true,alpha:false}); miniR.setPixelRatio(Math.min(2,devicePixelRatio||1)); miniR.shadowMap.enabled=true; if(miniHost)miniHost.appendChild(miniR.domElement);
...
// No ground shadow: keep the large popper clean and symmetrical.
const sm=(c,m=.25,q=.35)=>new T.MeshStandardMaterial({color:c,metalness:m,roughness:q});
function mesh(g,m,y){const x=new T.Mesh(g,m);x.position.y=y;x.castShadow=x.receiveShadow=false;scene.add(x);return x}
...
mesh(new T.CylinderGeometry(4.05,4.05,.13,64),sm(0xb9cfa7,0,.9),.01);
...
function diceClack(intensity=.12){audio||=new(window.AudioContext||window.webkitAudioContext)();const t=audio.currentTime,len=Math.floor(audio.sampleRate*.025),buf=audio.createBuffer(1,len,audio.sampleRate),data=buf.getChannelData(0);for(let i=0;i<len;i++)data[i]=(Math.random()*2-1)*(1-i/len);const src=audio.createBufferSource(),f=audio.createBiquadFilter(),g=audio.createGain();src.buffer=buf;f.type='highpass';f.frequency.value=700;g.gain.setValueAtTime(intensity,t);g.gain.exponentialRampToValueAtTime(.001,t+.025);src.connect(f).connect(g).connect(audio.destination);src.start(t)}
...
function step(dt){for(const d of dice){if(d.rest===true)continue;if(d.rest==='settling'){d.settleT+=dt;d.m.position.y=T.MathUtils.lerp(d.m.position.y,F,Math.min(1,12.034*dt));const ang=d.m.quaternion.angleTo(d.targetQ);d.m.quaternion.slerp(d.targetQ,Math.min(1,dt*(4.125+Math.max(0,.45-ang)*3.4375)));if(ang<=.009&&d.settleT>=.291){d.m.position.y=F;d.m.quaternion.copy(d.targetQ);d.v.set(0,0,0);d.w.set(0,0,0);d.rest=true}continue}d.t+=dt;d.v.y-=9.8*dt;d.m.position.addScaledVector(d.v,dt);const a=d.w.length()*dt;if(a)d.m.quaternion.premultiply(new T.Quaternion().setFromAxisAngle(d.w.clone().normalize(),a));const cx=d.m.position.x,cy=d.m.position.y-.08,cz=d.m.position.z,rad3=Math.hypot(cx,cy,cz),limit=DR-RR;
 if(cy>=0&&rad3>limit){const n=new T.Vector3(cx,cy,cz).normalize();d.m.position.set(n.x*limit,n.y*limit+.08,n.z*limit);const out=d.v.dot(n);if(out>0)d.v.addScaledVector(n,-1.72*out);d.v.multiplyScalar(.96)}if(d.m.position.y<F){if(d.v.y<-.9)diceClack(Math.min(.13,.035+Math.abs(d.v.y)*.012));d.m.position.y=F;if(d.v.y<0)d.v.y=-d.v.y*.50;d.v.x*=.915;d.v.z*=.915;d.w.multiplyScalar(.84)}if(d.t>.55&&d.w.length()<5.2){d.w.x+=(Math.random()-.5)*1.15*dt;d.w.z+=(Math.random()-.5)*1.15*dt}if(d.m.position.y<F+.22){const pr=T.MathUtils.clamp((F+.22-d.m.position.y)/.22,0,1);d.w.multiplyScalar(Math.pow(.14,dt*pr))}if(d.t>.38&&d.m.position.y<=F+.035&&Math.abs(d.v.y)<.32&&Math.hypot(d.v.x,d.v.z)<.34&&d.w.length()<1.9){
