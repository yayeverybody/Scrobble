...
function haptic(style){try{const H=window.Capacitor?.Plugins?.Haptics;if(H?.impact)H.impact({style});else if(navigator.vibrate)navigator.vibrate(style==='HEAVY'?45:18)}catch(e){}}
...
function pop(){if(rolling)return;rolling=true;push.disabled=true;status.textContent='Rolling…';dice.forEach(reset);
 compressionSound();haptic('LIGHT');
 const start=performance.now(),releaseAt=150;let released=false;
 (function a(){const x=performance.now()-start;if(x<releaseAt){const k=x/releaseAt;dome.scale.set(1-.1*k,1-.27*k,1-.1*k);dome.position.y=.08-.2*k}else{if(!released){released=true;releaseSound();haptic('HEAVY')}const k=Math.min(1,(x-releaseAt)/230),e=1-(1-k)**3;dome.scale.set(.9+.1*e,.73+.27*e,.9+.1*e);dome.position.y=-.12+.2*e}if(x<380)requestAnimationFrame(a);else{dome.scale.set(1,1,1);dome.position.y=.08}})()}
